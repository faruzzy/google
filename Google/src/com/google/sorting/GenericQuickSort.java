...
public class GenericQuickSort {
	...
	public static <T extends Comparable<T>> void quickSort(final Comparable<T>[] data) {
	...
	@SuppressWarnings("unchecked")
	private static <T extends Comparable<T>> void quickSort(final Comparable<T>[] data, final int left, final int right) {
		if (right - left < 1)
			return;
		if (right == left + 1) {
